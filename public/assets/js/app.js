...
function applyTheme(){
  ...
  const b=$('theme-btn'); if(b)b.innerHTML=darkMode?ICONS.sun:ICONS.moon;
}
...
// ── Two roles ─────────────────────────────────────────────────────────────────
const ROLES={
  admin: {label:'Administrator',    bc:'b-admin'},
  staff: {label:'Staff / Technician',bc:'b-staff'},
};
const PERMS={
...
// ── Equipment icon map ────────────────────────────────────────────────────────
const EQICONS={
  ac:       {bg:'#eff6ff',svg:`<svg width="20" height="20" fill="none" stroke="#3b82f6" stroke-width="1.8" viewBox="0 0 24 24"><rect x="2" y="7" width="20" height="10" rx="2"/><path d="M6 17v2M10 17v2M14 17v2M18 17v2M6 7V5M18 7V5"/></svg>`},
  light:    {bg:'#fffbeb',svg:`<svg width="20" height="20" fill="none" stroke="#f59e0b" stroke-width="1.8" viewBox="0 0 24 24"><path d="M9 21h6M12 3a6 6 0 016 6c0 2.5-1.5 4.5-3 6H9c-1.5-1.5-3-3.5-3-6a6 6 0 016-6z"/></svg>`},
  projector:{bg:'#fef2f2',svg:`<svg width="20" height="20" fill="none" stroke="#ef4444" stroke-width="1.8" viewBox="0 0 24 24"><rect x="2" y="7" width="20" height="10" rx="2"/><circle cx="17" cy="12" r="2"/><path d="M5 12h5"/></svg>`},
  fan:      {bg:'#f5f3ff',svg:`<svg width="20" height="20" fill="none" stroke="#8b5cf6" stroke-width="1.8" viewBox="0 0 24 24"><circle cx="12" cy="12" r="2"/><path d="M12 2a4 4 0 014 4c0 1.5-.5 3-2 4M12 22a4 4 0 01-4-4c0-1.5.5-3 2-4M2 12a4 4 0 014-4c1.5 0 3 .5 4 2M22 12a4 4 0 01-4 4c-1.5 0-3-.5-4-2"/></svg>`},
  hvac:     {bg:'#ecfeff',svg:`<svg width="20" height="20" fill="none" stroke="#0891b2" stroke-width="1.8" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M12 8v8M8 12h8"/></svg>`},
  fridge:   {bg:'#f0fdf4',svg:`<svg width="20" height="20" fill="none" stroke="#22c55e" stroke-width="1.8" viewBox="0 0 24 24"><rect x="5" y="2" width="14" height="20" rx="2"/><path d="M5 10h14M9 6v2M9 14v4"/></svg>`},
  computer: {bg:'#eff6ff',svg:`<svg width="20" height="20" fill="none" stroke="#3b82f6" stroke-width="1.8" viewBox="0 0 24 24"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>`},
  other:    {bg:'#f8fafc',svg:`<svg width="20" height="20" fill="none" stroke="#64748b" stroke-width="1.8" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M12 8v4l3 3"/></svg>`},
};
...
const ICONS={
  ...
  cal:       `<svg width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>`,
...
// ── Chart ─────────────────────────────────────────────────────────────────────
function renderChart(labels, values, cid, h=200){
  const el=$(cid); if(!el) return;
  const max=Math.max(...values,1), W=800, H=h;
...
function renderBarChart(labels, values, cid){
  const el=$(cid); if(!el) return;
  const max=Math.max(...values,1);
  el.innerHTML=`<div style="display:flex;align-items:flex-end;gap:8px;height:160px;padding:8px 0 4px">
    ${values.map((v,i)=>{
      const pct=Math.round(v/max*100);
      return`<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px">
        <span style="font-size:10px;color:var(--text-muted)">${parseFloat(v).toFixed(1)}</span>
        <div style="width:100%;height:${Math.max(pct,4)}%;background:var(--green);border-radius:4px 4px 0 0;min-height:6px;opacity:.85"></div>
...
// Demo data (shown when DB has no readings yet)
const DEMO={
  daily(){
    const v=[800,720,680,660,640,620,720,1100,2200,3800,4600,5200,5432,5100,4800,4400,3900,3400,2800,2200,1800,1400,1100,900];
    const l=Array.from({length:24},(_,i)=>{const h=i%12||12,s=i<12?'AM':'PM';return`${h}:00 ${s}`;});
    return{labels:l,values:v};
  },
  weekly(){return{labels:['Mon','Tue','Wed','Thu','Fri','Sat','Sun'],values:[38200,42100,35000,44800,48300,29100,22400]};},
  monthly(){
    const l=Array.from({length:30},(_,i)=>`${i+1}`);
    const v=[38,42,35,44,48,29,22,40,45,52,38,42,35,44,48,29,22,40,45,38,42,35,44,48,29,22,40,45,38,42].map(x=>x*1000);
    return{labels:l,values:v};
  },
  hourly:[620,540,490,470,460,480,780,1820,3200,4100,4800,5100,5300,5000,4700,4300,3800,3200,2600,2100,1700,1300,990,740],
  rooms:[
    {room_name:'Server Room',  equipment_label:'HVAC',          avg_power:4800,peak_power:5200,total_kwh:72.0,anomaly_count:0},
    {room_name:'Room 204',     equipment_label:'Air Conditioner',avg_power:2850,peak_power:5012,total_kwh:42.1,anomaly_count:2},
...
const NAV=[
  ...
  {id:'rooms',     label:'Rooms / Equipment',   icon:'room',      admin:true},
  {id:'monitoring',label:'Real-time Monitoring',icon:'monitor'},
  {id:'anomalies', label:'Anomalies',           icon:'alert',     badge:true},
  {id:'analytics', label:'Analytics',           icon:'analytics'},
  {id:'reports',   label:'Reports',             icon:'report'},
  {id:'thresholds',label:'Thresholds',          icon:'threshold', admin:true},
  {id:'users',     label:'Users',               icon:'users',     admin:true},
  {id:'logs',      label:'Logs',                icon:'logs',      admin:true},
  {id:'settings',  label:'Settings',            icon:'settings',  admin:true},
  {id:'profile',   label:'My Profile',          icon:'profile'},
];

const PAGE_TITLES={
  dashboard:'Dashboard',rooms:'Rooms / Equipment',monitoring:'Real-time Monitoring',
  anomalies:'Anomalies',analytics:'Analytics',reports:'Reports',
  thresholds:'Thresholds',users:'User Management',logs:'System Logs',
  settings:'Settings',profile:'My Profile',
};
const PAGE_SUBS={
  dashboard:'Welcome back, ',rooms:'Manage monitored rooms and devices',
  monitoring:'Live electricity readings from all locations',
  anomalies:'Detected power anomalies and threshold violations',
  analytics:'Hourly patterns, weekly trends, room breakdown & forecast',
  reports:'Consumption summaries by period',
  thresholds:'Set power consumption limits for anomaly detection',
  users:'Manage system user accounts and access',
  logs:'Activity and security audit trail',
  settings:'Configure WattWatch system parameters',
  profile:'Manage your account',
};

function navigate(page){
  if(!(PERMS[state.user?.role_key]||[]).includes(page)) return;
  state.page=page;
  $$('.nav-item').forEach(el=>el.classList.toggle('active',el.dataset.page===page));
  const tl=$('topbar-title'),ts=$('topbar-sub');
  if(tl) tl.textContent=PAGE_TITLES[page]||'WattWatch';
  if(ts) ts.textContent=(PAGE_SUBS[page]||'')+(page==='dashboard'?(state.user?.full_name||''):'');
  const pages={
    dashboard:renderDashboard, rooms:renderRooms, monitoring:renderMonitoring,
    anomalies:renderAnomalies, analytics:renderAnalytics, reports:renderReports,
    thresholds:renderThresholds, users:renderUsers, logs:renderLogs,
...
async function renderDashboard(){
  const pc=$('page-content'); pc.innerHTML=spinner();
  let stats=null; try{stats=await api('dashboard_stats');}catch{}
  ...
  pc.innerHTML=`
    <div class="stats-row">
      <div class="stat-card">
        <div class="stat-icon-wrap" style="background:var(--icon-bg-green)">
          <svg width="26" height="26" fill="#22c55e" viewBox="0 0 24 24"><path d="M13 3L4 14h7v7l9-11h-7V3z"/></svg>
        </div>
      ...
      <div class="stat-card">
        <div class="stat-icon-wrap" style="background:var(--icon-bg-yellow)">${ICONS.cal}</div>
    ...
    <div class="dash-bot">
      ...
      <div class="activity-panel">
        <div class="panel-header"><h3>Recent Activity</h3><a href="#" onclick="navigate('logs');return false" style="font-size:12px;color:var(--green);font-weight:600">View all</a></div>
        <div id="activity-list"></div>
      </div>
    </div>
    <p class="pg-footer">WattWatch v1.0 · © ${now.getFullYear()} All rights reserved · Role: ${ROLES[state.user?.role_key]?.label||''}</p>`;

  renderChart(DEMO.daily().labels,DEMO.daily().values,'main-chart',230);

  // Room cards
  const rc=$('room-cards');
  rc.innerHTML=rooms.length?rooms.map(r=>{
    const isA=r.status==='anomaly';
    const ri=EQICONS[r.icon_key||'other']||EQICONS.other;
    return`<div class="room-card ${isA?'anomaly':''}">
      ...
      <div class="room-power ${isA?'bad':''}">${parseFloat(r.power_watts||0).toFixed(0)} W</div>
      <div class="room-status"><div class="status-dot ${isA?'bad':'ok'}"></div><span>${isA?'Anomaly':'Normal'}</span></div>
    </div>`;
  }).join(''):`<p style="font-size:13px;color:var(--text-muted);padding:16px 0">No rooms yet — add rooms and connect the ESP32 to see live data.</p>`;

  // Anomalies
  try{
    const list=await api('get_anomalies',{status:'active'});
    $('dash-anomalies').innerHTML=list.slice(0,2).map(a=>`
      <div class="anom-item">
        ...
        <div class="anom-item-body">
          <div class="anom-item-top"><strong>${a.room_name} – ${a.equipment_label}</strong>${badge(a.type_label,'b-high')}</div>
      ...
      </div>`).join('')||'<p style="font-size:12px;color:var(--text-muted);padding:14px 0">No active anomalies.</p>';
  }catch{ $('dash-anomalies').innerHTML='<p style="font-size:12px;color:var(--text-muted);padding:12px 0">Could not load.</p>'; }

  // Activity
  const actIcons={auth:{bg:'#eff6ff',svg:`<svg width="14" height="14" fill="none" stroke="#3b82f6" stroke-width="2" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>`},anomaly:{bg:'#fffbeb',svg:`<svg width="14" height="14" fill="none" stroke="#f59e0b" stroke-width="2" viewBox="0 0 24 24"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/></svg>`},room:{bg:'#f0fdf4',svg:`<svg width="14" height="14" fill="none" stroke="#22c55e" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/></svg>`},report:{bg:'#eff6ff',svg:`<svg width="14" height="14" fill="none" stroke="#3b82f6" stroke-width="2" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/></svg>`},settings:{bg:'#f5f3ff',svg:`<svg width="14" height="14" fill="none" stroke="#8b5cf6" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/></svg>`},system:{bg:'#f8fafc',svg:`<svg width="14" height="14" fill="none" stroke="#64748b" stroke-width="2" viewBox="0 0 24 24"><rect x="2" y="3" width="20" height="14" rx="2"/></svg>`}};
  try{
    const logs=await api('get_logs');
    $('activity-list').innerHTML=logs.slice(0,5).map(l=>{
      const ic=actIcons[l.log_type]||actIcons.system;
      const t=l.logged_at?l.logged_at.split(' ')[1]?.substr(0,5):'';
      return`<div class="activity-item"><div class="activity-icon" style="background:${ic.bg}">${ic.svg}</div><div class="activity-body"><p>${l.action}</p></div><span class="activity-time">${t}</span></div>`;
    }).join('')||'<p style="font-size:12px;color:var(--text-muted);padding:10px 0">No recent activity.</p>';
  }catch{ $('activity-list').innerHTML='<p style="font-size:12px;color:var(--text-muted);padding:10px 0">Could not load.</p>'; }
}

// ── Rooms ─────────────────────────────────────────────────────────────────────
async function renderRooms(){
  $('page-content').innerHTML=`
    <div class="page-action-bar"><button class="btn btn-primary" onclick="showRoomForm()">${ICONS.plus} Add Room</button></div>
    ...
    <div class="tbl-wrap"><table>
      ...
      <tbody id="rooms-tbody">${spinner()}</tbody>
    </table></div>`;
  loadRoomsTable();
}
...
async function loadRoomsTable(){
  try{
    const rows=await api('get_rooms'); state.rooms=rows;
    $('rooms-tbody').innerHTML=rows.map(r=>`
      <tr>
        <td class="fw7">${r.room_name}</td><td>${r.equipment_label}</td>
        <td style="color:var(--text-muted)">${r.building_name}</td>
        ...
        <td><div class="flex gap-6">
          ...
          <button class="btn btn-danger btn-sm" onclick="deleteRoom(${r.room_id},'${r.room_name}')">${ICONS.trash} Delete</button>
        </div></td>
...
async function renderMonitoring(){
  $('page-content').innerHTML=`
    <div class="page-action-bar"><div id="esp32-badge" class="esp32-badge esp32-offline">${ICONS.wifioff} ESP32 Offline</div></div>
    <div class="grid-monitor">
      <div class="card" style="height:fit-content">
        <div style="padding:12px 14px;border-bottom:1px solid var(--border)"><p style="font-size:11px;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:.5px">Select Location</p></div>
        <div id="monitor-list"></div>
      </div>
      <div id="monitor-detail">${spinner()}</div>
    </div>`;
  checkEsp32();
  try{
    ...
    else $('monitor-detail').innerHTML=`<div class="card card-body"><p style="color:var(--text-muted)">No rooms configured yet.</p></div>`;
  }catch{ $('monitor-detail').innerHTML=`<div class="card card-body"><p style="color:var(--text-muted)">Could not load rooms.</p></div>`; }
}

async function checkEsp32(){
  const b=$('esp32-badge'); if(!b)return;
  try{
    const r=await api('get_readings',{room_id:1,limit:1});
    if(r&&r.length&&(Date.now()-new Date(r[0].read_at).getTime())<120000){
      b.className='esp32-badge esp32-online'; b.innerHTML=`${ICONS.wifi} ESP32 Online`; return;
    }
  }catch{}
  b.className='esp32-badge esp32-offline'; b.innerHTML=`${ICONS.wifioff} ESP32 Offline`;
}
...
async function selectMonitor(id){
    ...
    <div class="card card-body mb-16">
      ...
      <div class="metric-grid">
        ${[['Voltage',`${parseFloat(r.voltage||220).toFixed(1)} V`,'#3b82f6','#eff6ff'],
          ...
          ].map(([l,v,c,bg])=>`<div class="metric-box" style="background:${bg}">
  ...
  switchMonitorChart('daily',id);
}

function switchMonitorChart(period,id){
  ...
  const cd=DEMO[period]?.()||DEMO.daily();
  ...
  $('page-content').innerHTML=`
    <div class="page-action-bar">
      <span style="font-size:11px;color:var(--text-muted);display:flex;align-items:center;gap:5px">
        ...
        Resolved anomalies are automatically removed after 30 days
      </span>
    </div>
    ...
    <div id="anom-list">${spinner()}</div>`;
  loadAnomList('all');
}

async function loadAnomList(filter){
  const el=$('anom-list'); if(!el)return;
  const canResolve=['admin','staff'].includes(state.user?.role_key);
  try{
    ...
    el.innerHTML=list.length?list.map(a=>{
      ...
      return`<div class="anom-card ${isA?'active-anom':''}">
        ...
        <div style="flex:1">
          ...
          <p style="font-size:12.5px;color:var(--text-secondary)">Current: <strong>${parseFloat(a.power_at_event).toLocaleString()} W</strong> · Threshold: ${parseFloat(a.threshold_used).toLocaleString()} W · Exceeded by ${pct}%</p>
        ...
        ${isA&&canResolve?`<button class="btn btn-secondary btn-sm" onclick="resolveAnom(${a.anomaly_id})">${ICONS.check} Resolve</button>`:''}
      </div>`;
    }).join(''):`<div class="empty-state"><p>No ${filter==='all'?'':filter} anomalies found.</p></div>`;
...
async function renderAnalytics(){
  const pc=$('page-content'); pc.innerHTML=spinner();
  let hourlyData=DEMO.hourly, weekDays=['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], weekKwh=[38.2,42.1,35.0,44.8,48.3,29.1,22.4];
  let rooms=DEMO.rooms, weekTotal=260.0, dailyAvg=37.1, rate=6.00;
  let aStats={total:4,total_excess:5693,first_hour:10,last_hour:14}, topRoom={room_name:'Room 204',equipment_label:'Air Conditioner',cnt:2};
  let isLive=false;

  try{
    const d=await api('get_analytics');
    if(d.hourly_pattern?.some(v=>v>0)){hourlyData=d.hourly_pattern;isLive=true;}
    if(d.weekly?.length){weekDays=d.weekly.map(w=>w.day_name.substr(0,3));weekKwh=d.weekly.map(w=>parseFloat(w.total_kwh));}
    ...
    if(d.anomaly_stats)    aStats=d.anomaly_stats;
    if(d.top_room)         topRoom=d.top_room;
  }catch{}

  api('cleanup_anomalies',null,'POST').catch(()=>{});

  const peakDay=weekDays[weekKwh.indexOf(Math.max(...weekKwh))]||'—';
  const monthForecast=(dailyAvg*30).toFixed(1);
  const monthlyCost=(parseFloat(monthForecast)*rate).toFixed(2);
  const avgExcess=aStats.total>0?Math.round(parseFloat(aStats.total_excess||0)/aStats.total):0;
  const totalKwh=rooms.reduce((s,r)=>s+parseFloat(r.total_kwh||0),0)||1;

  pc.innerHTML=`
    ${!isLive?`<div style="margin-bottom:14px;padding:10px 14px;background:var(--icon-bg-yellow);border:1px solid var(--yellow);border-radius:var(--radius);font-size:12px;color:var(--text-secondary)"><strong>Demo data shown</strong> — connect the ESP32 and send readings to see live analytics.</div>`:''}
    ...
    <div class="stats-row mb-20">
      ...
      <div class="stat-card">
        ...
        <div class="stat-text"><label>Peak Day</label><h2>${peakDay}</h2><small>${Math.max(...weekKwh).toFixed(1)} kWh</small></div>
      </div>
      <div class="stat-card">
        ...
        <div class="stat-text"><label>Est. Monthly Cost</label><h2>₱${parseFloat(monthlyCost).toLocaleString()}</h2><small>@ ₱${rate}/kWh</small></div>
      </div>
    </div>
    ...
    <div class="card card-body mb-16">
      ...
      <div class="tbl-wrap" style="box-shadow:none;border:1px solid var(--border)">
        <table>
          <thead><tr><th>Room</th><th>Equipment</th><th>Avg Power</th><th>Peak Power</th><th>Energy (kWh)</th><th>% Share</th><th>Anomalies</th></tr></thead>
          <tbody>${rooms.map(r=>{
            const share=(parseFloat(r.total_kwh||0)/totalKwh*100).toFixed(1);
            const bw=Math.min(100,Math.round(parseFloat(r.avg_power||0)/6000*100));
            const bc=bw>80?'prog-over':bw>60?'prog-warn':'prog-ok';
            return`<tr>
              <td class="fw7">${r.room_name}</td>
              <td style="color:var(--text-secondary)">${r.equipment_label}</td>
              <td><div style="display:flex;align-items:center;gap:8px"><span class="fw7">${parseFloat(r.avg_power||0).toLocaleString()} W</span><div style="width:56px">${prog(bw,bc)}</div></div></td>
              <td>${parseFloat(r.peak_power||0).toLocaleString()} W</td>
              <td>${parseFloat(r.total_kwh||0).toFixed(1)}</td>
              <td>${share}%</td>
    ...
    <div class="grid-2">
      <div class="card card-body">
        ...
        ${[['Projected Monthly Usage',monthForecast+' kWh','30 × daily avg'],['Estimated Monthly Cost','₱'+parseFloat(monthlyCost).toLocaleString(),'@ ₱'+rate+'/kWh'],['Weekly Average',(weekTotal/7).toFixed(1)+' kWh/day','Last 7 days']].map(([l,v,s])=>`
      ...
      <div class="card card-body">
        <div class="card-title">Anomaly Analysis (Last 7 Days)</div>
        ${[['Total Anomalies Detected',aStats.total+' events'],['Most Affected Room',topRoom?(topRoom.room_name+' – '+topRoom.equipment_label):'None'],['Peak Anomaly Window',aStats.total>0?(aStats.first_hour+':00 – '+aStats.last_hour+':00'):'—'],['Avg Excess Power',aStats.total>0?(avgExcess.toLocaleString()+' W over limit'):'—']].map(([l,v])=>`
          <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border)">
            <span style="font-size:13px;color:var(--text-secondary)">${l}</span>
  ...
  renderBarChart(weekDays,weekKwh,'weekly-bar');
}

// ── Reports ───────────────────────────────────────────────────────────────────
async function renderReports(){
  $('page-content').innerHTML=`
    <div class="page-action-bar"><button class="btn btn-primary" onclick="window.print()">${ICONS.download} Export</button></div>
    <div class="filter-tabs">
...
async function loadReport(period,btn){
  $$('.ftab').forEach(e=>e.classList.remove('active')); if(btn)btn.classList.add('active');
  const el=$('report-body'); if(!el)return;
  el.innerHTML=spinner();
  const defs={daily:{energy:45.67,peak:5432,anomalies:2,rooms:7},weekly:{energy:312.5,peak:5800,anomalies:5,rooms:7},monthly:{energy:1256.34,peak:6200,anomalies:12,rooms:7}};
  const def=defs[period]; const cd=DEMO[period]?.()||DEMO.daily();
  let totalEnergy=def.energy,peakPower=def.peak,anomalyCount=def.anomalies,roomsMonitored=def.rooms,byRoom=[];
  try{
    const d=await api('get_report',{period}); const s=d.summary||{};
    totalEnergy=parseFloat(s.total_energy)||totalEnergy; peakPower=parseFloat(s.peak_power)||peakPower;
    anomalyCount=parseInt(d.anomaly_count)??anomalyCount; roomsMonitored=parseInt(s.rooms_monitored)||roomsMonitored;
    byRoom=d.by_room||[];
  }catch{}
  const label={daily:'Today',weekly:'This Week',monthly:'This Month'}[period];
  el.innerHTML=`
    <div class="stats-row mb-16">
      <div class="stat-card"><div class="stat-text"><label>Total Energy</label><h2>${totalEnergy.toFixed(2)} kWh</h2><small>${label}</small></div></div>
      ...
      <div class="stat-card"><div class="stat-text"><label>Anomalies</label><h2>${anomalyCount}</h2><small>${label}</small></div></div>
    ...
    <div class="chart-card mb-16"><div class="chart-header"><h3>Power Consumption — ${label}</h3></div><div id="rep-chart"></div></div>
    <div class="card card-body">
      <div class="card-title">Consumption by Room</div>
      ${(byRoom.length?byRoom:[{room_name:'Server Room',equipment_label:'HVAC',avg_power:4800},{room_name:'Room 204',equipment_label:'Air Cond.',avg_power:2850},{room_name:'Computer Lab',equipment_label:'Room 103',avg_power:1240},{room_name:'Cafeteria',equipment_label:'Fridges',avg_power:950},{room_name:'Room 101',equipment_label:'Projector',avg_power:420},{room_name:'Room 201',equipment_label:'Fan',avg_power:110}]).map(r=>{
...
async function renderThresholds(){
  $('page-content').innerHTML=`
    ...
    <div class="tbl-wrap"><table>
      ...
      <tbody id="thresh-tbody">${spinner()}</tbody>
    </table></div>`;
  loadThreshTable();
}
  ...
  try{
    ...
    $('thresh-tbody').innerHTML=rooms.length?rooms.map(r=>{
      ...
      return`<tr>
        ...
        <td id="td-th-${r.room_id}"><strong>${th.toLocaleString()} W</strong></td>
        <td style="min-width:160px">${prog(pct,cls)}<span style="font-size:11px;color:${pw>th?'var(--red)':'var(--text-muted)'};margin-top:3px;display:block">${pct}% of limit</span></td>
        <td><div class="flex gap-6">
          <button class="btn btn-blue btn-sm" onclick="editThresh(${r.room_id},${th})">${ICONS.edit} Edit</button>
          <button class="btn btn-danger btn-sm" onclick="deleteRoom(${r.room_id},'${r.room_name}')">${ICONS.trash} Remove</button>
        </div></td>
      </tr>`;
...
function editThresh(id,cur){
  $(`td-th-${id}`).innerHTML=`<div class="flex gap-6">
...
async function renderUsers(){
  $('page-content').innerHTML=`
    <div class="page-action-bar"><button class="btn btn-primary" onclick="showUserForm()">${ICONS.plus} Add User</button></div>
...
async function loadUsersList(){
  try{
    ...
    $('users-list').innerHTML=users.map(u=>{
      const rl=ROLES[u.role_key]||ROLES.staff; const isMe=u.user_id==state.user?.user_id;
      return`<div class="user-card">
        ...
        <div class="u-right">
          <div class="u-badges">${badge(rl.label,rl.bc)} ${badge(u.status,u.status==='active'?'b-normal':'b-inactive')}</div>
          <div class="u-btns">
      ...
      <div class="form-grid">
        ...
        <div class="form-group"><label>Role</label>
          <select id="uf-role" class="form-ctrl" onchange="updatePermPrev()">
            <option value="admin" ${u?.role_key==='admin'?'selected':''}>Administrator — Full control</option>
            <option value="staff" ${u?.role_key==='staff'||!u?'selected':''}>Staff / Technician — Monitoring &amp; reports</option>
          </select>
        </div>
      </div>
      <div style="margin-top:8px;padding:12px 14px;background:var(--bg-secondary);border-radius:var(--radius)">
        <p style="font-size:11px;font-weight:600;color:var(--text-muted);margin-bottom:8px">Access for selected role:</p>
        <div id="perm-prev" class="perm-tags">${(PERMS[u?.role_key||'staff']||[]).map(p=>`<span class="perm-tag">${p.replace('_',' ')}</span>`).join('')}</div>
      </div>
      <div class="form-actions">
...
function updatePermPrev(){
  ...
  $('perm-prev').innerHTML=(PERMS[r]||[]).map(p=>`<span class="perm-tag">${p.replace('_',' ')}</span>`).join('');
}
...
async function saveUser(id){
  ...
  if(!id)d.password=$('uf-pw')?.value||'';
  if(!id&&d.password.length<6)return showAlert('user-alert','Password must be at least 6 characters.','error');
  try{
...
async function renderLogs(){
  $('page-content').innerHTML=`<div class="tbl-wrap"><div id="logs-wrap">${spinner()}</div></div>`;
  try{
    const logs=await api('get_logs');
      ...
      <div class="card card-body">
        ...
        ${[['alert_email','Email Alerts','Send alerts via email on anomaly'],
           ...
           ['alert_buzzer','Buzzer / LED Alert','Trigger hardware buzzer on ESP32'],
            ...
            <div><h5>${l}</h5><p style="color:var(--text-muted);font-size:12px">${d}</p></div>
...
async function saveSettings(){
  const d={system_name:$('s-name')?.value||'WattWatch',timezone:$('s-tz')?.value||'Asia/Manila',refresh_rate:$('s-ref')?.value||'5',data_retention:$('s-ret')?.value||'90',kwh_rate:$('s-rate')?.value||'6.00',alert_email:$('tog-alert_email')?.checked?'1':'0',alert_dashboard:$('tog-alert_dashboard')?.checked?'1':'0',alert_buzzer:$('tog-alert_buzzer')?.checked?'1':'0'};
  try{await api('save_settings',d,'POST');showAlert('set-alert','Settings saved!');}catch(e){showAlert('set-alert',e.message,'error');}
}

// ── Profile ───────────────────────────────────────────────────────────────────
async function renderProfile(){
  const u=state.user; const rl=ROLES[u.role_key]||ROLES.staff;
  $('page-content').innerHTML=`
    <div class="grid-2">
      <div class="card card-body">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:20px;padding-bottom:18px;border-bottom:1px solid var(--border)">
          <div class="u-avatar" style="width:52px;height:52px;border-radius:14px;font-size:18px">${u.avatar}</div>
          <div><h3 style="font-size:17px;font-weight:800;margin:0 0 4px">${u.full_name}</h3>${badge(rl.label,rl.bc)}</div>
        </div>
      ...
      <div style="display:flex;flex-direction:column;gap:16px">
        ...
        <div class="card card-body">
          <div class="card-title">${ICONS.shield} Access Permissions</div>
          <p style="font-size:12px;color:var(--text-muted);margin-bottom:10px">${u.role_key==='admin'?'Full control over user accounts, threshold configurations, system maintenance, and security audit logs.':'Restricted to real-time energy monitoring, viewing historical trends/reports, and acknowledging or clearing anomaly alerts.'}</p>
          <div class="perm-tags">${(PERMS[u.role_key]||[]).map(p=>`<span class="perm-tag">${p.replace('_',' ')}</span>`).join('')}</div>
        </div>
      </div>
    </div>`;
}
...
function buildShell(user){
  ...
  <div class="app-shell">
    <nav class="sidebar">
      <div class="sidebar-logo" onclick="navigate('dashboard')" title="Go to Dashboard">
      ...
      <div class="sidebar-bottom">
        ...
        ${!isAdmin?`<div class="help-box" style="margin-top:12px">
          <div class="help-title">${ICONS.help} Need Help?</div>
          <p>Contact your administrator for assistance.</p>
        </div>`:''}
      </div>
      <div class="sidebar-footer">WattWatch v1.0 · © ${new Date().getFullYear()} All rights reserved.</div>
    </nav>
    ...
    <div class="main-area">
        ...
        <div class="topbar-right">
          ...
          <div style="position:relative">
            <button class="user-chip" onclick="toggleDD()">
              ...
              <div class="user-info-text"><strong id="topbar-uname">${user.full_name}</strong><span>${ROLES[user.role_key]?.label||''}</span></div>
            ...
            <div class="user-dropdown hidden" id="user-dd">
              <div class="dropdown-head">
                ...
                <span style="font-size:11px;color:var(--text-muted);margin-top:4px;display:inline-block">${ROLES[user.role_key]?.label||''}</span>
              </div>
      ...
      <div class="page-content" id="page-content">${spinner()}</div>
    </div>
  </div>
    ...
    <div class="login-wrap">
      ...
      <div class="login-card">
        ...
        <div class="login-demo">
          ...
          <button class="demo-btn" onclick="fillDemo('admin@wattwatch.com','admin123')"><strong>Administrator</strong> — admin@wattwatch.com</button>
          <button class="demo-btn" onclick="fillDemo('staff@wattwatch.com','juan123')"><strong>Staff / Technician</strong> — staff@wattwatch.com</button>
        </div>
      </div>
      <p style="text-align:center;font-size:11px;color:var(--text-muted);margin-top:16px">WattWatch v1.0 · Isabela State University · IT 313</p>
    </div>
  </div>`;
  applyTheme();
}
