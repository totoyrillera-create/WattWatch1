/* ============================================================
   WattWatch — app.js  (pixel-matched to mockup)
   ============================================================ */
'use strict';
...
/* ── Role config ────────────────────────────────────────────── */
const ROLE_STYLE = {
  admin:            { label:'Administrator',    avatarBg:'#fef2f2', avatarColor:'#dc2626', bc:'b-admin'   },
  facility_manager: { label:'Facility Manager', avatarBg:'#eff6ff', avatarColor:'#2563eb', bc:'b-manager' },
  technician:       { label:'Technician',       avatarBg:'#fffbeb', avatarColor:'#d97706', bc:'b-tech'    },
  viewer:           { label:'Viewer',           avatarBg:'#f9fafb', avatarColor:'#6b7280', bc:'b-viewer'  },
};
const ROLE_PERMISSIONS = {
  admin:            ['dashboard','rooms','monitoring','anomalies','reports','thresholds','users','logs','settings','profile'],
...
/* ── SVG icon strings ───────────────────────────────────────── */
const ICONS = {
  bolt:      `<svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24"><path d="M13 3L4 14h7v7l9-11h-7V3z"/></svg>`,
...
function renderChart(labels, values, containerId, height=200){
  ...
  el.innerHTML=`
    ...
    <div class="chart-wrap">
      <div class="chart-y">
        ${yLabels.map(v=>`<span>${v.toLocaleString()}</span>`).join('')}
      </div>
...
function demoChart(){
  ...
  const full  =[800,660,720,3800,5200,4800,2800,1100];
  // 24 points for smooth curve
  ...
  return {labels:lbs,values:vals,xLabels:labels,xVals:full};
}
...
async function renderDashboard(){
  ...
  const isAdmin    = state.user?.role_key==='admin';
  const cd         = demoChart();
  const now        = new Date();
    ...
    <div class="stats-row">
      <div class="stat-card">
        <div class="stat-icon-wrap" style="background:#f0fdf4">
          <svg width="26" height="26" fill="none" stroke="#22c55e" stroke-width="2" viewBox="0 0 24 24"><path d="M13 3L4 14h7v7l9-11h-7V3z" fill="#22c55e" stroke="none"/></svg>
        </div>
        <div class="stat-text">
          <label>Total Power (Now)</label>
          <h2 class="text-green">${totalPower.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g,',')} W</h2>
          <small>Updated: ${timeStr}</small>
        </div>
      </div>
      <div class="stat-card">
        ...
        <div class="stat-text">
          ...
          <h2 class="text-blue">${todayE} kWh</h2>
          <small>From 12:00 AM</small>
        </div>
      </div>
      <div class="stat-card">
        ...
        <div class="stat-text">
          <label>Total Energy (This Month)</label>
          <h2 class="text-yellow">${monthE} kWh</h2>
          <small>${now.toLocaleDateString('en-US',{month:'long',day:'numeric'})} – ${dateStr}</small>
        </div>
      </div>
      <div class="stat-card">
        ...
        <div class="stat-text">
          ...
          <h2 class="text-red">${activeAnom}</h2>
          <small>Requires attention</small>
        </div>
      </div>
    </div>

    <!-- Chart + Anomalies panel -->
    <div class="dash-mid">
      <div class="chart-card">
        <div class="chart-header">
          <h3>Power Consumption Over Time</h3>
          <div class="chart-pill">
            ${isAdmin?'<span class="live-dot"></span> Live':'<span class="live-dot"></span> Today'}
      ...
      <div class="anom-panel">
        <div class="panel-header">
          ...
          <a href="#" onclick="navigate('anomalies');return false">View all</a>
        </div>
        <div id="dash-anomalies"></div>
        <button class="btn-view-all-anom" onclick="navigate('anomalies')">
          View All Anomalies ${ICONS.arrow}
        </button>
      </div>
    </div>

    <!-- Room cards + Activity panel -->
    <div class="dash-bot">
      ...
      <div class="activity-panel">
        <div class="panel-header">
          <h3>${isAdmin?'Recent Activity':'My Recent Activity'}</h3>
  ...
  // Render chart
  renderChart(cd.labels, cd.values, 'main-chart', 230);

  // Room cards
  const rcEl=$('room-cards');
  if(rooms.length){
    rcEl.innerHTML=rooms.map(r=>{
      ...
      return `<div class="room-card ${isAnom?'anomaly':''}">
        ...
        <h4>${r.room_name}</h4>
        <p>${r.equipment_label}</p>
        <div class="room-power ${isAnom?'bad':'ok'}">${parseFloat(r.power_watts||0).toFixed(0)} W</div>
        <div class="room-status">
          <div class="status-dot ${isAnom?'bad':'ok'}"></div>
          <span style="color:${isAnom?'var(--red)':'var(--slate-600)'}">${isAnom?'Anomaly':'Normal'}</span>
        </div>
      </div>`;
    }).join('');
  } else {
    rcEl.innerHTML=`<p class="text-muted" style="font-size:13px;padding:20px 0">No rooms yet. Add rooms and connect the ESP32 to see live data.</p>`;
  }
...
async function loadDashAnomalies(){
  ...
  try{
    ...
    el.innerHTML=list.slice(0,2).map(a=>`
      <div class="anom-item">
        <div class="anom-item-icon">
          <svg width="18" height="18" fill="none" stroke="#ef4444" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 8v4"/><circle cx="12" cy="16" r="1" fill="#ef4444"/></svg>
        </div>
        <div class="anom-item-body">
          <div class="anom-item-top">
            <strong>${a.room_name} – ${a.equipment_label}</strong>
            <span class="badge b-high">${a.type_label}</span>
          </div>
  ...
  }catch{ el.innerHTML='<p class="text-muted" style="font-size:12px;padding:12px 0">Could not load anomalies.</p>'; }
}

async function loadDashActivity(){
  const el=$('activity-list'); if(!el) return;
  // Activity icons per log type
  const actIcon={
  ...
  }catch{ el.innerHTML='<p class="text-muted" style="font-size:12px;padding:12px 0">Could not load activity.</p>'; }
}
...
function showRoomForm(id=null){
  ...
  $('room-form-wrap').innerHTML=`
    <div class="form-panel">
      <h3>${r?'Edit Room':'Add New Room'}</h3>
      <div class="form-grid">
...
async function renderMonitoring(){
  ...
  try{
    ...
    const ml=$('monitor-list');
    ml.innerHTML=state.rooms.map(r=>`
  ...
  $('monitor-detail').innerHTML=`
    <div class="card card-body mb-16">
      <div class="flex-bc mb-16">
        ...
        <p class="text-muted" style="font-size:12px;margin-top:3px">${r.building_name} · Updated live</p></div>
      ...
      <div class="metric-grid">
        ...
        ].map(([l,v,c,bg])=>`<div class="metric-box" style="background:${bg}"><label style="color:${c}">${l}</label><div class="mval" style="color:${c}">${v}</div></div>`).join('')}
      </div>
    </div>
    <div class="chart-card">
      <div class="chart-header">
        <h3>Power Trend (Today)</h3>
        <span style="font-size:12px;color:var(--slate-500)">Threshold: ${parseFloat(r.threshold_watts||0).toLocaleString()} W</span>
      </div>
...
async function resolveAnom(id){
  try{await api('resolve_anomaly',{anomaly_id:id},'POST');loadAnomList('all');}catch(e){alert(e.message);}
}
...
async function loadReport(period,btn){
  ...
  try{
    ...
    const s=d.summary; const cd=demoChart();
    el.innerHTML=`
      <div class="stats-row mb-16">
        ${[['Total Energy',(parseFloat(s.total_energy)||45.67).toFixed(2)+' kWh','blue','#eff6ff'],
           ['Peak Power',(parseFloat(s.peak_power)||5432).toLocaleString()+' W','yellow','#fffbeb'],
           ['Anomalies',d.anomaly_count,'red','#fef2f2'],
           ['Rooms',s.rooms_monitored||7,'green','#f0fdf4']
      ...
      <div class="card card-body">
        ...
        ${(d.by_room||[]).map(r=>{const pct=Math.min(100,Math.round((parseFloat(r.avg_power||0)/6000)*100));const cls=pct>80?'prog-over':pct>60?'prog-warn':'prog-ok';return `
          ...
          </div>`}).join('')||'<p class="text-muted" style="font-size:13px">No data for this period.</p>'}
      </div>`;
...
async function saveThresh(id){
  ...
  try{await api('set_threshold',{room_id:id,threshold_watts:v},'POST');showAlert('thresh-alert','Threshold updated!');loadThreshTable();}catch(e){showAlert('thresh-alert',e.message,'error');}
}
  ...
  try{
    ...
    $('users-list').innerHTML=users.map(u=>{
      ...
      return `<div class="user-card">
        ...
        <div class="u-right">
          <div class="u-badges">
            ${badge(rs.label,rs.bc)} ${badge(u.status,u.status==='active'?'b-normal':'b-inactive')}
          </div>
          <div class="u-btns">
...
async function renderLogs(){
  const pc=$('page-content');
  pc.innerHTML=`<div class="page-hdr"><h1>System Logs</h1><p class="text-muted">Complete activity and audit trail</p></div>
...
function buildShell(user){
  ...
  // Build nav — combine roles + altRoles
  const navHTML = NAV_DEF.filter(n=>{
    const allRoles=[...(n.roles||[]),...(n.altRoles||[])];
    return allRoles.includes(user.role_key);
  }).map(n=>{
  ...
  <div class="app-shell">
    ...
    <nav class="sidebar">
      <div class="sidebar-logo">
        <div class="logo-bolt"><svg width="18" height="18" fill="white" viewBox="0 0 24 24"><path d="M13 3L4 14h7v7l9-11h-7V3z"/></svg></div>
      ...
      <div class="sidebar-bottom">
        ...
        ${user.role_key==='facility_manager'||user.role_key==='technician'||user.role_key==='viewer'?`
      ...
      <div class="sidebar-footer">
        WattWatch v1.0<br>© ${new Date().getFullYear()} All rights reserved.
        <button class="btn-logout" onclick="logout()">${ICONS.logout} Sign Out</button>
      </div>
    </nav>
      ...
      <div class="topbar">
        ...
        <div class="topbar-right">
          ...
          <div style="display:flex;align-items:center;gap:8px;padding:5px 10px;border:1px solid var(--slate-200);border-radius:var(--radius);font-size:12px;color:var(--slate-600)">
            ${ICONS.calendar} <span id="topbar-daterange">May 16 – May 23, 2025</span>
          ...
          <div style="position:relative">
            ...
            <div class="user-dropdown hidden" id="user-dd">
              <div class="dropdown-head">
                ...
                <span class="badge ${rs.bc}" style="margin-top:5px">${rs.label}</span>
              </div>
              <button class="dd-item" onclick="navigate('profile');toggleDD()">${ICONS.profile} My Profile</button>
              <div class="dd-divider"></div>
              <button class="dd-item danger" onclick="logout()">${ICONS.logout} Sign Out</button>
            </div>
          </div>
        </div>
      </div>
  ...
  // Anomaly badge poll
  const pollBadge=async()=>{try{const list=await api('get_anomalies',{status:'active'});const n=list.length;const bb=$('bell-badge'),nb=$('nav-anom-badge');if(bb){bb.textContent=n;bb.classList.toggle('hidden',n===0);}if(nb){nb.textContent=n;nb.classList.toggle('hidden',n===0);}}catch{}};
  pollBadge(); setInterval(pollBadge,30000);

  // Click outside dropdown
  document.addEventListener('click',e=>{const dd=$('user-dd');if(dd&&!dd.classList.contains('hidden')&&!e.target.closest('[onclick="toggleDD()"]')&&!e.target.closest('#user-dd'))dd.classList.add('hidden');});

  navigate('dashboard');
}

function toggleDD(){const dd=$('user-dd');if(dd)dd.classList.toggle('hidden');}
...
async function doLogin(){
  ...
  try{
    ...
    state.user=user; buildShell(user);
  }catch(e){
